...
class LintingResultHandler(
    ...
    fun handleLintingResult(result: LintingResultEvent) {
        ...
        if (result.isValid) {
            ...
            snippet.lastValidationError = result.violations.joinToString("\n") {
                "Line ${it.line}, Column ${it.column}: ${it.message}"
            }
        }

        snippetRepository.save(snippet)
        println("✅ [Snippet Service] Snippet ${result.snippetId} compliance updated: ${snippet.complianceStatus}")
    }
}